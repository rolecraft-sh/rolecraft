...
export async function convertApi(source, options = {}) {
  ...
  if (skillFile) {
    return [await convertSingleFile(skillFile, 'skill', outDir, options)]
  }
  ...
  if (mdcFiles.length > 0) {
    ...
    for (const mf of mdcFiles) {
      results.push(await convertSingleFile(mf, 'mdc', outDir, options))
    }
    return results
  }
...
async function convertSingleFile(inputPath, format, outDir, options) {
  ...
  if (options.dryRun) {
    if (format === 'skill') {
      ...
      return {
        dryRun: true,
        from: inputPath,
        to: join(outDir, `${slug}.mdc`),
      }
    } else {
      return {
        dryRun: true,
        from: inputPath,
        to: join(outDir, 'SKILL.md'),
      }
    }
  }
  ...
  if (format === 'skill') {
    ...
    return { from: inputPath, to: outPath, format: 'skill-to-mdc' }
  } else {
    const outPath = join(outDir, 'SKILL.md')
    await writeFile(outPath, mdcToSkill(content, basename(inputPath)), 'utf-8')
    return { from: inputPath, to: outPath, format: 'mdc-to-skill' }
  }
}
