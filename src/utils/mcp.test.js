...
describe('mcp', () => {
  ...
  describe('addMcpServer', () => {
    it(
      'adds an MCP server to agents (opencode) config',
      withTempDir(async () => {
        ...
        assert.deepEqual(config.mcpServers['test-server'].args, [
          '-y',
          '@test/server',
        ])
      }),
    ...
    it(
      'adds an MCP server to cursor config',
      withTempDir(async () => {
    ...
    it(
      'adds an MCP server to claude config',
      withTempDir(async () => {
    ...
    it(
      'adds an MCP server with env vars',
      withTempDir(async () => {
    ...
    it(
      'adds an MCP server to copilot format',
      withTempDir(async () => {
    ...
    it(
      'adds an MCP server to continue format',
      withTempDir(async () => {
        ...
        const found = config.experimental.mcpServers.find(
          (s) => s.name === 'continue-server',
        )
        assert.ok(found)
    ...
    it(
      ...
      withTempDir(async () => {
        ...
        assert.deepEqual(config.mcpServers['update-test'].args, [
          '-y',
          '@test/v2',
        ])
      }),
    ...
    it('returns false for unsupported agent', async () => {
      const success = await mcpModule.addMcpServer(
        'nonexistent-agent',
        'test',
        {
          command: 'npx',
  ...
  describe('updateMcpServer', () => {
    it(
      'replaces existing MCP server configuration',
      withTempDir(async () => {
        ...
        assert.deepEqual(config.mcpServers['my-server'].args, [
          '-y',
          '@test/v2',
        ])
      }),
    ...
    it(
      ...
      withTempDir(async () => {
        ...
        try {
          ...
          const success = await mcpModule.updateMcpServer(
            'copilot',
            'copilot-update',
            {
              command: 'npx',
          ...
          const config = JSON.parse(
            readFileSync(
              join(process.env.HOME, '.github', 'copilot', '.mcp.json'),
              'utf-8',
            ),
          )
          assert.deepEqual(config.servers['copilot-update'].args, [
            '-y',
            '@test/v2',
          ])
        } finally {
    ...
    it(
      'updates continue format server',
      withTempDir(async () => {
        ...
        const config = JSON.parse(
          readFileSync(
            join(process.env.HOME, '.continue', 'config.json'),
            'utf-8',
          ),
        )
        const found = config.experimental.mcpServers.find(
          (s) => s.name === 'cont-update',
        )
        assert.ok(found)
  ...
  describe('removeMcpServer', () => {
    it(
      'removes an MCP server from config',
      withTempDir(async () => {
    ...
    it(
      ...
      withTempDir(async () => {
        ...
        try {
          ...
          const success = await mcpModule.removeMcpServer(
            'copilot',
            'copilot-rm',
          )
          assert.ok(success)
    ...
    it(
      'removes continue format server',
      withTempDir(async () => {
    ...
    it(
      'returns false when server does not exist',
      withTempDir(async () => {
        const success = await mcpModule.removeMcpServer(
          'agents',
          'non-existent',
  ...
  describe('listMcpServers', () => {
    it(
      'returns empty array when no servers configured',
      withTempDir(async () => {
...
`
    ...
    it('returns empty array when no frontmatter', () => {
      const servers = mcpModule.parseMcpServersFromSkill(
  ...
  describe('resolveMcpSource', () => {
    ...
    it('resolves npm: source with @version (scoped)', () => {
      ...
      assert.deepEqual(resolved.args, [
        '-y',
        '@modelcontextprotocol/github@1.2.3',
      ])
      assert.equal(resolved.sourceType, 'npm')
  ...
  describe('readMcpConfig', () => {
    ...
    it(
      'returns empty data for non-existent config',
      withTempDir(async () => {
  ...
  describe('setExecSync', () => {
    it('replaces the execSync implementation', () => {
      const _origExec = mcpModule.setExecSync(() => 'mocked')
      const mockFn = () => 'mocked'
  ...
  describe('installMcpServersFromSkill', () => {
    it('returns empty array when no mcp_servers in content', async () => {
      const results = await mcpModule.installMcpServersFromSkill(
        'no servers here',
        ['agents'],
      )
    ...
    it(
      'installs MCP servers for given targets',
      withTempDir(async () => {
        const content = `---
name: test
mcp_servers:
...
---
`
        const results = await mcpModule.installMcpServersFromSkill(content, [
          'agents',
        ])
  ...
  describe('mcp-lock', () => {
    ...
    describe('readMcpLock and writeMcpLock', () => {
      it(
        'returns default for missing lockfile',
        withTempDir(async () => {
      ...
      it(
        'writes and reads lockfile',
        withTempDir(async () => {
          const data = {
            version: 1,
            servers: { test: { source: 'npm:@test/pkg', agents: ['cursor'] } },
          }
    ...
    describe('addServerToMcpLock', () => {
      it(
        ...
        withTempDir(async () => {
          await lockModule.addServerToMcpLock('my-server', {
            source: 'npm:@test/pkg',
            agents: ['cursor'],
          })
          const lock = await lockModule.readMcpLock()
      ...
      it(
        'merges agents when adding same server again',
        withTempDir(async () => {
          await lockModule.addServerToMcpLock('my-server', {
            source: 'npm:@test/pkg',
            agents: ['cursor'],
          ...
          const lock = await lockModule.readMcpLock()
          assert.deepEqual(lock.servers['my-server'].agents.sort(), [
            'claude',
            'cursor',
          ])
        }),
    ...
    describe('removeServerFromMcpLock', () => {
      ...
      it(
        'deletes entry when last agent removed',
        withTempDir(async () => {
          await lockModule.addServerToMcpLock('my-server', {
            source: 'npm:@test/pkg',
            agents: ['cursor'],
      ...
      it(
        'no-op for non-existent server',
        withTempDir(async () => {
          const lock = await lockModule.removeServerFromMcpLock(
            'nonexistent',
            'cursor',
          )
  ...
  describe('lockfile integration', () => {
    ...
    it(
      'addMcpServer creates lockfile entry',
      withTempDir(async () => {
        mcpModule.addMcpServer('agents', 'locked-server', {
          command: 'npx',
          args: ['-y', '@test/locked'],
        })
    ...
    it(
      'removeMcpServer removes agent from lockfile',
      withTempDir(async () => {
        await mcpModule.addMcpServer('agents', 'to-remove-lock', {
          command: 'npx',
          args: ['-y', '@test/rm'],
        })
        await new Promise((r) => setTimeout(r, 50))
    ...
    it(
      'installMcpServersFromSkill records source in lockfile',
      withTempDir(async () => {
        const content = `---
name: test
mcp_servers:
...
`
        ...
        assert.equal(
          lock.servers['lock-skill-mcp'].source,
          'npm:@test/lock-skill',
        )
      }),
    )
