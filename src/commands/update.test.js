...
describe('update command', () => {
  ...
  it('detects skill in windsurf target directory', async () => {
    ...
    writeFileSync(
      join(tempDir, 'wind-source', 'SKILL.md'),
      '# slug: wind/skill\nname: Wind Skill\nContent',
    )
  ...
  it('detects skill in devin target directory', async () => {
    ...
    writeFileSync(
      join(tempDir, 'dev-source', 'SKILL.md'),
      '# slug: dev/skill\nname: Dev Skill\nContent',
    )

    mkdirSync(join(tempDir, '.devin', 'skills', 'dev-skill'), {
      recursive: true,
    })
    writeFileSync(
      join(tempDir, '.devin', 'skills', 'dev-skill', 'SKILL.md'),
      '# slug: dev/skill\nname: Dev Skill\nContent',
    )
  ...
  it('finds skill by normalized slug when exact match fails', async () => {
    ...
    writeFileSync(
      join(tempDir, 'dash-source', 'SKILL.md'),
      '# slug: dash/skill\nname: Dash Skill\nContent',
    )

    mkdirSync(join(tempDir, '.agents', 'skills', 'dash-skill'), {
      recursive: true,
    })
    writeFileSync(
      join(tempDir, '.agents', 'skills', 'dash-skill', 'SKILL.md'),
      '# slug: dash/skill\nname: Dash Skill\nContent',
    )
  ...
  it('falls back to agents target when no existing targets detected', async () => {
    ...
    writeFileSync(
      join(tempDir, 'fresh-source', 'SKILL.md'),
      '# slug: fresh/skill\nname: Fresh Skill\nContent',
    )
  ...
  it('finds skill by name part when slug does not match directly or normalized', async () => {
    ...
    writeFileSync(
      join(tempDir, 'tool-source', 'SKILL.md'),
      '# slug: mygroup/tool\nname: My Tool\nContent',
    )

    mkdirSync(join(tempDir, '.agents', 'skills', 'mygroup-tool'), {
      recursive: true,
    })
    writeFileSync(
      join(tempDir, '.agents', 'skills', 'mygroup-tool', 'SKILL.md'),
      '# slug: mygroup/tool\nname: My Tool\nContent',
    )
  ...
  it('detects skill in claude target directory', async () => {
    ...
    writeFileSync(
      join(tempDir, 'cla-source', 'SKILL.md'),
      '# slug: cla/skill\nname: Claude Skill\nContent',
    )

    mkdirSync(join(tempDir, '.claude', 'skills', 'cla-skill'), {
      recursive: true,
    })
    writeFileSync(
      join(tempDir, '.claude', 'skills', 'cla-skill', 'SKILL.md'),
      '# slug: cla/skill\nname: Claude Skill\nContent',
    )
  ...
  it('detects skill in cursor target directory', async () => {
    ...
    writeFileSync(
      join(tempDir, 'cur-source', 'SKILL.md'),
      '# slug: cur/skill\nname: Cursor Skill\nContent',
    )

    mkdirSync(join(tempDir, '.cursor', 'skills', 'cur-skill'), {
      recursive: true,
    })
    writeFileSync(
      join(tempDir, '.cursor', 'skills', 'cur-skill', 'SKILL.md'),
      '# slug: cur/skill\nname: Cursor Skill\nContent',
    )
  ...
  it('detects skill in codex target directory', async () => {
    ...
    writeFileSync(
      join(tempDir, 'cod-source', 'SKILL.md'),
      '# slug: cod/skill\nname: Codex Skill\nContent',
    )
  ...
  it('detects skill in aider target directory', async () => {
    ...
    writeFileSync(
      join(tempDir, 'aid-source', 'SKILL.md'),
      '# slug: aid/skill\nname: Aider Skill\nContent',
    )

    mkdirSync(join(tempDir, '.aider', 'skills', 'aid-skill'), {
      recursive: true,
    })
    writeFileSync(
      join(tempDir, '.aider', 'skills', 'aid-skill', 'SKILL.md'),
      '# slug: aid/skill\nname: Aider Skill\nContent',
    )
  ...
  it('detects skill in gemini target directory', async () => {
    ...
    writeFileSync(
      join(tempDir, 'gem-source', 'SKILL.md'),
      '# slug: gem/skill\nname: Gemini Skill\nContent',
    )

    mkdirSync(join(tempDir, '.gemini', 'skills', 'gem-skill'), {
      recursive: true,
    })
    writeFileSync(
      join(tempDir, '.gemini', 'skills', 'gem-skill', 'SKILL.md'),
      '# slug: gem/skill\nname: Gemini Skill\nContent',
    )
