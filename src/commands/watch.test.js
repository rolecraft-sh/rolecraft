...
before(async () => {
  ...
  await mkdir(join(tempDir, '.agents', 'skills', 'local-skill'), {
    recursive: true,
  })
  writeFileSync(
    join(tempDir, '.agents', 'skills', 'local-skill', 'SKILL.md'),
    '---\nname: Local Skill\nslug: local-skill\n---\nContent',
  )
...
describe('watch command', () => {
  ...
  it('shows message when no local skills available', async () => {
    ...
    try {
      await mkdir(join(remoteOnlyDir, '.agents', 'skills', 'gh-skill'), {
        recursive: true,
      })
      writeFileSync(
        join(remoteOnlyDir, '.agents', 'skills', 'gh-skill', 'SKILL.md'),
        '---\nname: GH Skill\nslug: gh-skill\n---\nContent',
      )
      await writeFile(
        join(remoteOnlyDir, '.agents', '.skill-lock.json'),
        JSON.stringify({
          version: 3,
          skills: {
            'gh-skill': {
  ...
  it('handles file system watcher error gracefully', async () => {
    ...
    try {
      ...
      await writeFile(
        join(badDir, '.agents', '.skill-lock.json'),
        JSON.stringify({
          version: 3,
          skills: {
            'bad-skill': {
  ...
  it('reinstalls skill on file change and logs sync', async () => {
    ...
    try {
      ...
      assert.ok(
        ...
        `Expected 'syncing' in logs: ${logs.join('; ')}`,
      )
      assert.ok(
        logs.some((l) => l.includes('synced')),
        `Expected 'synced' in logs: ${logs.join('; ')}`,
      )
    } finally {
      console.log = origLog
    }
  ...
  it('handles reinstall failure gracefully', async () => {
    ...
    try {
      ...
      await writeFile(
        join(dir, 'broken-source', 'SKILL.md'),
        '---\nname: FailSkill\nslug: fail-skill\n---\nContent',
      )
      await writeFile(
      ...
      assert.ok(
        !logs.some((l) => l.includes('synced')),
        'Should NOT log synced on failure',
      )
      assert.ok(
        logs.some((l) => l.includes('sync failed')),
        `Expected 'sync failed' in logs: ${logs.join('; ')}`,
      )
    } finally {
      console.log = origLog
      process.env.HOME = tempDir
